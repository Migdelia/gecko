...
foreach($itens_guia as $key=>$val) {
	...
	//SOMA TODAS AS DIFERENCAS do FECHAMENTO
	...
	if($resultado_dif_fecha['valor_desconto'] == "")
...

	{
		...
		$diferenca = $resultado_dif_fecha['valor_desconto'];
	...
	//$fat = $resultado_fat['faturamento'] - $resultado_dif_fecha['valor_desconto'];
	...
	$guias_cont.="\n\t\t\t\t<td align='center' colspan='5'>";
		...
		$guias_cont.="\n\t\t\t\t<table border='1' width='100%' bordercolor='#000000' style='border-collapse: collapse'>";
...

			{
				...
				//$sub_total = $sub_total + $dados_leit['fat_cidade'] - $res_dif['valor_desconto'];
...
*/
	...
	$guias_cont.="\n\t\t\t\t<td align='center' align='center' colspan='5'>";
		...
		$guias_cont.="\n\t\t\t\t<table border='1' width='100%' bordercolor='#000000' style='border-collapse: collapse'>";
			...
			*/	
			...
			$query_ger=@mysql_query($sql_pct_ger);
			$res_ger=@mysql_fetch_assoc($query_ger);			

			$com_ger = ($fat * $res_ger['pct_gerente']) / 100;
			//$com_ger = ($fat * 5) / 100;
	...
	/*
			...
			*/
			...
			// $desc_socio = ((((($fat * 80) / 100) - $com_ger - $tot_gastos) *50) /100) + $vl_tot_porc_socio;			Desabilitado Erico
			$desc_socio = 0;
			...
			$vl_tot_porc_propria = $fat - $com_ger - $tot_gastos - $desc_socio;	
				...
				//desativado comissao do operador

				/*

				$com_ope = ($vl_tot_porc_propria * $resultado_fecha['pct_operador']) / 100;

				

				if($id_local == 49 or $id_local == 55 or $id_local == 54)
		...
		//fim comissao operador
			...
			$guias_cont.="\n\t\t\t\t<tr bgcolor='#FFFFFF'>";
				...
				//echo $vl_tot_porc_socio;


				$porc_maquinas_cala = ($sub_total * 20) / 100;

				//$porc_maquinas_cala = $porc_maquinas_cala - $vl_tot_porc_socio;
