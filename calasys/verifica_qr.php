...
if( ($_POST['login']!='')&&($_POST['senha']!='') ){
	...
	if( ($_POST['login']==$existe['usuario'])&&(md5($senhaCrip)==md5($existe['senha'])) ){
		...
		$jsc.="\n\twindow.location='novaValidacao.php?s=".$senhaQR."&v=".$codVer."';";
		$jsc.="\n</script>";
		$des ="<a href='novaValidacao.php?s=".$senhaQR."&v=".$codVer."' title='Acessar o sistema' style='text-decoration:none'><strong><img src='img/alerta.png' title='Acessar o sistema' align='absmiddle' border=none alt='Acessar o sistema' /> Acessar o sistema</strong></a>";
	}else{
		//Gravando dados no log de acessos
		...
		$jsc.="\n\twindow.location='sair_qr.php?s=".$senhaQR."&v=".$codVer."';";
		$jsc.="\n</script>";
		$des ="<a href='index_qr.php?s=".$senhaQR."&v=".$codVer."' title='Sair do sistema' style='text-decoration:none'><strong><img src='img/alerta.png' title='Sair do sistema' align='absmiddle' border=none alt='Sair do sistema' /> Sair do Sistema</strong></a>";
	}
}else{
	//Gravando dados no log de acessos
	$dados = array('login'=>$_POST['login'],'senha'=>$senhaCrip,'status'=>'falha','link'=>$_SERVER['REQUEST_URI']);
