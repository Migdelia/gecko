...
class IAM_XLS {
      ...
      */
     function WriteValue($xls_row, $xls_col, $value,$type=0)
     {
          if( $type!=0 )
...
//			  if (is_numeric($value))
			  if (is_numeric($value)&&(strlen($value)!=11))
				   $this->WriteCellNumber($xls_row, $xls_col, RetirarAcentos ($value) );
			  else
				   $this->WriteCellText($xls_row, $xls_col, RetirarAcentos ($value) );
     }
...
//               }
			   ...
			   $this->WriteValue(0, $e, trim(ucwords(mysql_field_name($result, $e))));
               ...
               for($col = 0; $col < $colums; $col++) {
                    ...
                    for($i = 0; $i < $lines; $i++) {
                         ...
                         $xls_line = ($i + 1);
						 if( strpos($col_name,"CNPJ")!==false )
							 $this->WriteValue($xls_line, $col, $CellValue,1);
						 else
