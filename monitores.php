...
$limitRegistros = mysql_num_rows($query_mon);
...
<html>
  ...
  <body class="body innerpages">
    ...
    <?php $file_name = "monitores" // ingresar la palabra clave de cada modal ?>
                    ...
                    <form id="formDados" >
                      ...
                      <table id="idTabela" class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <td class="center-align"><?php echo $res_mon['serie'] ?></td>
                            <td class="center-align"><?php echo $res_mon['descricao'] ?></td>
                            <td class="center-align"><?php echo $res_maquina['numero'] ?></td>
                            <td class="center-align">
                              <a id="edit_<?php echo $res_mon['id_monitor'] ?>" href="#" class="btn btn-sm" data-toggle="modal" data-target="#edit-modal-<?php echo $filenameID ?>" onClick="alimentaEdit(this);" title="Editar"><i class="fa fa-edit"></i></a>
                              <a id="ver_<?php echo $res_mon['id_monitor'] ?>" href="#" class="btn btn-sm" data-toggle="modal" data-target="#view-modal-<?php echo $filenameID ?>" onClick="alimentaView(this);" title="Ver"><i class="fa fa-eye"></i></a>
                            </td>
                          </tr>
...
<script>
	...
	{
		var idMon = (obj.id).split("_");
		
		var $iframe = $('#view_monitor');
		$iframe.attr('src','frames/view-monitores.php?id='+idMon[1]);  		
	...
	{
		var idMonitor = (obj.id).split("_");

		//consulta dados do objeto para preencher modal.
		$.ajax(
		{								
			type: "POST", // Defino o método de envio POST / GET
			url: 'functions/consulta_dados_monitores.php', // Informo a URL que será pesquisada.
			data: 'id='+idMonitor[1],
			success: function(html)
			{
				...
				numMaq = arrayDados[2];
				idMaq = arrayDados[3];
				idMonitor = arrayDados[4];
				idModelo = arrayDados[5];

	
				$('#id_monitor').attr("value", idMonitor);
				$('#select_modMon_Monitor').text(modelo);
				$('#input_modMon_Monitor').attr("value", idModelo);
				$("#edit_serie_Monitor").attr("value", serie);
				$('#select_maqMon_Monitor').text(numMaq);
				$('#input_maqMon_Monitor').attr("value", idMaq);
