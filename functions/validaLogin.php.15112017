...
if( ($_SESSION['usuario']==$existe['usuario'])&&(md5($_SESSION['passw'])==md5($existe['senha']))&&($_SESSION['usuario']!='')&&($_SESSION['passw']!='') ){
	...
	if(@mysql_num_rows( $query_pg ) >= 1) {
		...
		if ( (@mysql_num_rows($query_a)== 0)||(strtoupper($result_pg['excluido'])=='S') ) {
			header('Location:functions/sair.php');
		}
	}
}else{
	session_destroy();
	...
	//verifica se venho do QR
	if($senhaQR == NULL)
	{
		$jsc.="\n\twindow.location.href='functions/sair.php'";
	}
	else
	{
		$jsc.="\n\twindow.location.href='functions/sair.php?s=" .$senhaQR ."&v=".$codVer."'";
	}
	$jsc.="\n</script>";
