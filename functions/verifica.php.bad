...
if( ($_POST['inputEmail']!=''))
{
	...
	if( ($_POST['inputEmail']==$existe_usuario['usuario']))
	{
		...
		//consulta senha
		$sql = @mysql_query("SELECT logins.id_login,logins.id_nivel, logins.usuario, logins.nome, logins.senha, logins.email, nivel.descricao FROM logins INNER JOIN nivel ON logins.id_nivel = nivel.id_nivel WHERE md5(logins.usuario)='".md5($_POST['inputEmail'])."' AND md5(logins.senha)='".md5($senhaCrip)."' AND logins.excluido='N'");	
		...
		if( ($_POST['inputEmail']==$existe['usuario'])&&(md5($senhaCrip)==md5($existe['senha'])) )
		{
			...
			$_SESSION['desc_nivel'] = $existe['descricao'];
