...
<script>
	...
	$('#confirmar').click( function ()
	{
		...
		while (cont< "<?=$NumeroLinhas?>")
		{
			...
			var obj_ent = id_obj + "_entrada";
			var vl_obj_ent = $("#" + obj_ent + "").text();
			
			//pega o valor de saida
			var obj_sai = id_obj + "_salida";
		...
		$("#confirmar").attr('disabled','disabled');
			...
			alert(ids_maq);
		//
		$.post('edit_leitura.php',{loc:id_loc,idleitura:leitura,ids:ids_maq,vl_maq:param_leitura,qtd:num_lin,dev:deve,sld_dev:saldo_deve,abono:abn_dev,fat_bruto:fatBruto},function(json){
			//
		
				$("#lc_nome").val("");
				//location="edit_leitura.php?id="+json;
