...
$limitRegistros = mysql_num_rows($query_bil);
...
<html>
  ...
  <body class="body innerpages">
    ...
    <?php $file_name = "billeteros" // ingresar la palabra clave de cada modal ?>
                      ...
                      <table id="idTabela" class="table table-striped table-hover">
                        <thead>
                          ...
                          <!---
                            ...
                            <th class="center-align">
                              <a href="#"><?php echo _('Descripcion') ?></a>
                            </th>
                            <th class="center-align">
                        ...
                        <tbody>
                            ...
                            <td class="center-align"><?php echo $res_bil['serie'] ?></td>
                            <td class="center-align"><?php echo $res_bil['descricao'] ?></td>
                            <td class="center-align"><?php echo $res_maquina['numero'] ?></td>
                            <td class="center-align">
                              <a id="edit_<?php echo $res_bil['id_bilheteiro'] ?>" href="#" class="btn btn-sm" data-toggle="modal" data-target="#edit-modal-<?php echo $filenameID ?>" onClick="alimentaEdit(this);"><?php echo _('Editar') ?></a>
                              <a id="ver_<?php echo $res_bil['id_bilheteiro'] ?>" href="#" class="btn btn-sm" data-toggle="modal" data-target="#view-modal-<?php echo $filenameID ?>" onClick="alimentaView(this);"><?php echo _('Ver') ?></a>
                            </td>
                          </tr>
...
<script>
	...
	{
		var idPlaca = (obj.id).split("_");
		
		var $iframe = $('#view_billetero');
		$iframe.attr('src','frames/view-billeteros.php?id='+idPlaca[1]);  		
	...
	{
		var idBil = (obj.id).split("_");

		//consulta dados do objeto para preencher modal.
		$.ajax(
		{								
			type: "POST", // Defino o método de envio POST / GET
			url: 'functions/consulta_dados_bil.php', // Informo a URL que será pesquisada.
			data: 'id='+idBil[1],
			success: function(html)
			{
				...
				$('#select_modBil_Billetero').text(modelo);
				$('#select_maqBil_Billetero').text(numMaq);
				$("#edit_serie_Billetero").attr("value", serie);
				
				
				$('#id_bilheteiro').attr("value", idBil);
				$('#input_modBil_Billetero').attr("value", idModelo);
				$('#input_maqBil_Billetero').attr("value", idMaq);
				
				
				/*
				$('#id_placa_mae').attr("value", idPlaca);
				$('#select_modPlac_Placa').text(modelo);
				$('#input_modPlac_Placa').attr("value", idModelo);
				$("#edit_serie_Placa").attr("value", serie);
				$('#select_maqPlac_Placa').text(numMaq);
				$('#input_maqPlac_Placa').attr("value", idMaq);
				*/
