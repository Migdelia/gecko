...
//Montando a Combo de Dongles
$ev_pesquisa="[";
while ( $acesso = @mysql_fetch_assoc($query_log) ) {
	$ev_pesquisa.= "'".($acesso['ds'])."',";
}
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function() {
			var oTable = $('#example').dataTable( {
				...
				"bProcessing": true,
				"bServerSide": true,
				"sAjaxSource": "functions/serverProcessingDongle.php?idc=<?=$_SESSION['campos']?>&idi=<?=base64_encode('MachineId')?>&idt=<?=base64_encode('StreetDongle')?>",
				"fnRowCallback": 
		...
		function fnOpenClose ( oSettings ) {
			...
			$('#example tbody tr td:nth-child(3)').editable( 'functions/editableAjaxDongle.php', {
