...
{
	$whr = " WHERE id_login = ".$_SESSION['id_login']." ";
}
...
$sql_log = "
	SELECT
		...
		vw_local.rut,
		vw_local.nome,
		vw_local.id_tp_local,
		vw_local.operador,
		vw_local.id_gerente,
		vw_local.pct_operador,
		vw_local.pct_gerente,
		vw_local.excluido
	FROM
		vw_local
	".$whr;
...
//Montando a Combo de maquinas
$ev_pesquisa="[";
while ( $acesso = @mysql_fetch_assoc($query_log) ) {
	$ev_pesquisa.= "'".($acesso['operador'])."',";
}
$ev_pesquisa = substr($ev_pesquisa,0,-1)."]";


//Lendo os Niveis Cadastrados
$sql_ope = "SELECT id_login,nome as operador FROM `vw_operador` ".$whr." ORDER BY `nome`";
$query_ope=@mysql_query($sql_ope);

//Montando os Valores da Combo editavel de Nivel
$ope = " {";
while( $opr=@mysql_fetch_assoc($query_ope) ) {
	$ope.= "'".$opr['id_login']."':'".$opr['operador']."', ";
}
$ope=substr($ope,0,-2)."}";


//criar uma array igual para tipo de local Aquii
//Lendo os Niveis Cadastrados
$sql_tp_lc = "SELECT id_tp_local,tp_local FROM `tipo_local` ORDER BY `tp_local`";
$query_tp_lc=@mysql_query($sql_tp_lc);

//Montando os Valores da Combo editavel de Nivel
$tp_lc = " {";
while( $tipo_local=@mysql_fetch_assoc($query_tp_lc) ) {
	$tp_lc.= "'".$tipo_local['id_tp_local']."':'".$tipo_local['tp_local']."', ";
}
$tp_lc=substr($tp_lc,0,-2)."}";
...
//declara o nivel do usuario para dar permissao.
$tabela.= "<input type='hidden' id='usNivel' name='usNivel' value='".$_SESSION['usr_nivel']."' />";
?>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function() {
			var oTable = $('#example').dataTable( {
				...
				"aoColumns": [ {"bSearchable":true,"bVisible":true},null,null,null,null,null,null,null,null],
				"aLengthMenu": [[10, 20 ], [10, 20 ]],
				"bProcessing": true,
				"bServerSide": true,
				"sAjaxSource": "functions/server_processing1.php?idc=<?=$_SESSION['campos']?>&idi=<?=base64_encode('id_local')?>&idt=<?=base64_encode('vw_local')?>",
				"fnRowCallback": 
		...
		function fnOpenClose ( oSettings ) {
			...
			$('#example tbody tr td:nth-child(7)').editable( 'functions/editable_ajax.php', {
				"callback": function( sValue, y ) {
					var aPos = oTable.fnGetPosition( this );
					oTable.fnUpdate( sValue, aPos[0], aPos[1] );
				},
			...
			$('#example tbody tr td:nth-child(8)').editable( 'functions/editable_ajax.php', {
				"callback": function( sValue, y ) {
					var aPos = oTable.fnGetPosition( this );
					oTable.fnUpdate( sValue, aPos[0], aPos[1] );
				},
			...
			*/
			...
			var userNivel =$('#usNivel').attr("value");
			userNivel = eval(userNivel);
			...
			?>
				$('#example tbody tr td:nth-child(9)').editable( 'functions/editable_ajax.php', {
					data	: "{'':'','Sim':'Excluir'}",
					type	: 'select',
					submit	: 'OK',
					cancel	: 'Cancel',
					width	: '80%',
					tooltip:	'Clique para excluir este Local.',
					"callback": function( sValue, y ) {
					...
					"height":"14px",
					"placeholder":"<b><img src='img/lixeira.png' alt='Excluir' title='Excluir este Local' /></b>"
				} );
			<?php			
			}
			?>
		...
		{
			location="parametros.php?flag=local";
		}		
