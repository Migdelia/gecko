...
while($res_maq=@mysql_fetch_assoc($query_maq)) 
{
	...
	//consulta dados
	$sql_tipo = "SELECT machineType FROM Config WHERE id = " . $res_maq['interface'];
	$query_tipo=@mysql_query($sql_tipo);
	$res_tipo=@mysql_fetch_assoc($query_tipo);	
	
	//trata servidor / cliente
	if($res_tipo['machineType'] == 0)
	{
		$tipoMaq = '(cliente)';		
	}
	else
