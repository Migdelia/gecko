...
<th style='color:#E17009;'>Total</th>
		...
		while($result_maq=@mysql_fetch_assoc($query_maq)) {
			...
			*/
			...
			$guias_cont.="\n\t\t\t\t\t\t <input type='text' name='" . $result_maq['id_maquina'] . "_sai' size='20' id='" . $result_maq['id_maquina'] . "_sai' style='text-align:center' onclick='selCampo(this);' onkeypress='verificaTecla(this);' onBlur='calcula(this.id," . $result_maq['id_maquina'] . "," . $result_maq['valor_entrada_total'] . "," . $result_maq['valor_saida_total'] ."," . $result_maq['porc_maquina'] . ");' title='".$cont."' value='".number_format($saida,0,"",".")."' > <input type='button' id='rep_sai_".$result_maq['id_maquina']."' name='rep_sai_".$result_maq['id_maquina']."' title='".$result_maq['valor_saida_total']."' value=' = ' onclick='rep_valor(this);' />";
			$guias_cont.="\n\t\t\t\t\t</td>\n";
			$coluna=5;
			$cont++;
...
<head>
	...
	$("#online").click( function () {
		location="http://186.67.7.147/arquivo/uploader_leitura_online.php?a=<?= $list_plaq ?>&b=<?= $cont_plaq ?>";
	});		
	
	 //importacion
	$("#rep_leit").click( function () {
		location="leitura_rua_completo.php?id=32&rep=1";
	});
