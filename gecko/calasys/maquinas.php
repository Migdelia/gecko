...
//ate aqui esta ok para editar visualizacao soh das maquinas desse operador





//Lendo a tabela Com as Interfaces

$sql_int = "

	SELECT

		vw_interface.id_interface,
	...
	AND
		...
		vw_interface.excluido = 'N'

	ORDER BY

		vw_interface.descricao
...
//Montando as Combos de Edição e Pesquisa da tela para Regional.

$interface="' ':'Nenhuma',";

while ( $dados_int=@mysql_fetch_assoc($query_int) ) {

	$interface.= "'".$dados_int['id_interface']."':'".$dados_int['descricao']."',";
...
while ( $dados_reg=@mysql_fetch_assoc($query_reg) ) {
	...
	$regional.= "'_".$dados_reg['id_local']."':'".$dados_reg['nome']."',";

	$re_pesquisa.= "'".RetirarAcentos($dados_reg['nome'])."',";

}



$re_pesquisa = substr($re_pesquisa,0,-1)."]";
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function() {
			...
			var oTable = $('#example').dataTable( {
				...
				"aoColumns": [ {"bSearchable":true,"bVisible":true},null,null,null,null,null,null,null],
				...
				"sAjaxSource": "functions/server_processing1.php?idc=<?=$_SESSION['campos']?>&idi=<?=base64_encode('id_maquina')?>&idt=<?=base64_encode('vw_maquinas')?>",
		...
		function fnOpenClose ( oSettings ) {
			...
			$('#example tbody tr td:nth-child(4)').editable( 'functions/editable_ajax.php', {
				...
				tooltip:	'Clique para excluir esta Maquina.',
			...
			$('#example tbody tr td:nth-child(5)').editable( 'functions/editable_ajax.php', {
				...
				$('#example tbody tr td:nth-child(8)').editable( 'functions/editable_ajax.php', {
					...
					tooltip:	'Clique para modificar a interface.',
					...
					"placeholder":"<b><img src='img/lixeira.png' alt='Excluir' title='Excluir esta Maquina' /></b>"
...

		{
			...
			location="parametros.php?flag=maquina";
...
<body>
	...
	<div id='addregistro'>
    	...
    	<a onclick="redireciona();" target="_blank" style="text-decoration:none;">
            ...
            <strong>Adicionar Maquinas</strong>
