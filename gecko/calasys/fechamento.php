...
//recebe tipo de local.
$idTpLocal = $_POST['tpLocal'];

if($idTpLocal == "")
{
	$idTpLocal = $_GET['tploc'];
}
...
{
	//verifica o tipo de local selecionado.
	if($idTpLocal == 2)//tipo de local (proprio com socio)
	{
		$sWhere = " AND local.id_gerente = ".$id_ope." ";
	}
	else
	{
		$sWhere = " AND (local.id_login = ".$id_ope." OR local.id_gerente = ".$id_ope.") ";
	}
...
//adicionar a condicao o tipo de local *$sWhere*
$sWhere.= " AND local.id_tp_local = ".$idTpLocal;
...
if ($gerar_re!='carregar') {
	...
	if ( $reg_existe <= 0 ) {	
		...
		//declara id tp local
		$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='tipoLocal' name='tipoLocal' value='".$idTpLocal."' > ";
		
		//declara lista de locais
		$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='lista_locais' name='lista_locais' value='/' > ";		
		...
		$conteudo.="\n\t\t\t\t<a id='confirmar_lancamentos' style='color:#FFFFFF;text-decoration:none;margin:0px; margin-left:10%;cursor:pointer;font-size:11px;background-color:#485F65;width:85px;height:36px;color:#FFFFFF;border:1px solid #FFFFFF;font-weight:bolder;display:inline-block;'>
		<br />&nbsp;
		<strong> &nbsp;Confirmar </strong><img src='img/ativo.png' width='10' border='0' /></a>";	
		...
		//echo "verificar quantos locais tem esse usuario <br />";
		
		//echo "id ope: " . $id_ope . "<br />";
		...
		$sql_qtd_locais = "
			...
			FROM
				local
			WHERE
				local.id_login = '".$id_ope."'
		";
	
		$query_qtd_local = @mysql_query($sql_qtd_locais);			
		$NumeroLinhas = mysql_num_rows($query_qtd_local);
		...
		$query_qtd_tp_local = @mysql_query($sql_qtd_tp_locais);			
		$NumeroLinhasTp = mysql_num_rows($query_qtd_tp_local);		
		...
		$query_qtd_tpd_local = @mysql_query($sql_qtd_tpd_locais);			
		$NumeroLinhasTpD = mysql_num_rows($query_qtd_tpd_local);
		...
		$query_qtd_tpt_local = @mysql_query($sql_qtd_tpt_locais);			
		$NumeroLinhasTpT = mysql_num_rows($query_qtd_tpt_local);
		...
		$query_qtd_tpq_local = @mysql_query($sql_qtd_tpq_locais);			
		$NumeroLinhasTpQ = mysql_num_rows($query_qtd_tpq_local);										
		
		/*
		echo "Total Locais: " . $NumeroLinhas . "<br />";
		echo "Total Rua: " . $NumeroLinhasTp . "<br />";	
		echo "Total Propeio com socio: " . $NumeroLinhasTpD . "<br />";	
		echo "Total Lan House: " . $NumeroLinhasTpT . "<br />";	
		echo "Total Proprio: " . $NumeroLinhasTpQ . "<br />";							
		*/		
		...
		if($idTpLocal == 1)
		{
			$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='qtd_locais' name='qtd_locais' value='".$NumeroLinhasTp."' > ";			
		}
		else if($idTpLocal == 2)
		{
			$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='qtd_locais' name='qtd_locais' value='".$NumeroLinhasTpD."' > ";			
		}
		else if($idTpLocal == 3)
		{
			$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='qtd_locais' name='qtd_locais' value='".$NumeroLinhasTpT."' > ";			
		}
		else if($idTpLocal == 4)
		{
			$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='qtd_locais' name='qtd_locais' value='".$NumeroLinhasTpQ."' > ";			
		}
		if($idTpLocal == 6)
		{
			$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='qtd_locais' name='qtd_locais' value='".$NumeroLinhasTp."' > ";			
		...
		{
			$conteudo.="\n\t\t\t\t\t\t\t&nbsp; <input type='hidden' id='qtd_locais' name='qtd_locais' value='".$NumeroLinhas."' > ";			
		}						
				...
				if ( $chv!='id_leitura' and  $chv!='id_local') {
					...
					*/
					...
					if($i == 1)
					{
						...
						else if($repasse['id_tp_local'] == 6) // rua
						{
							$despesas = $repasse['desconto'];
					...
					//Icone de Baixa
					$baixa_ico = "\n\t\t\t\t\t\t\t<input type='checkbox' name='chk_bx_".$repasse['id_leitura']."' id='chk_bx_".$repasse['id_leitura']."' title='".$fat_sem_dif."/".$repasse['deve']."/".$despesas."/".$repasse['valor_recebido']."/".$descontos_local. "' value='".$repasse['id_leitura']."' onclick='totalizador(this,".$repasse['id_local'].");'>";
...
<head>
	...
	<script language="javascript" type="text/javascript">
		...
		$(document).ready( function() {
			...
			//mostra mascara de selecao de tipo de local.
			
			var idTpLocal =$('#tipoLocal').attr("value");
			...
			$('#operador').change( function() {
				...
				location = "fechamento.php?ope="+id_ope+"&tploc="+idTpLocal; // passar tipo de local junto 
		...
		function totalizador(obj,idLocal) // aquiii
		{
			tpCheck = obj.id;
			...
			else
			{
				...
				{
					//alert("buscar string na array");
					var listaLocais =$('#lista_locais').attr("value");
					
					//monta valor a ser excluido
					var idExcluir = "/" + idLocal + "/";
					
					//dividi array no id do local.
					var novaListalocal = listaLocais.split(idExcluir);
					//monta nova array
					var novaArraylocais = novaListalocal[0] + "/" + novaListalocal[1];
			...
			if(obj.checked)
			{
				//adicionar o numero de id do local a Array * Erico
				//trazer valor do campo lista de locais
				var listaLocais =$('#lista_locais').attr("value");
				
				//verificar se o local ja esta marcado
				var contVeri = "/" + idLocal + "/";	
				var resVerf = listaLocais.indexOf(contVeri);			
				if(resVerf < 0)
				{
					//atribui e substirui lista de maquinas
					listaLocais = listaLocais + idLocal + "/";
					$("#lista_locais").attr("value", listaLocais);	
				}
				
				//alert(listaLocais);
				var qtdLocais = listaLocais.split("/");
				qtdLocais = qtdLocais.length - 2;
				//alert(qtdLocais); 
				
				// busca a quantidade de locais a ser marcado.
				var qtd_a_marcar =$('#qtd_locais').attr("value");
				//alert(qtd_a_marcar);
				
				//verifica se ja marcou a quantidade de local corretos
				//alert(qtdLocais);
				//alert(qtd_a_marcar);
				if(qtdLocais == qtd_a_marcar)
				{
		...
		function sumChecked() {
			...
			*/
			...
			//desativado para apare4cer soh quando selecionar a quantidade de locais necessario.
			//$('#confirmar_lancamentos').css('visibility','visible');
...
<body onblur="window.opener.location.reload();">
    ...
    <div id="boxes"> 
        ...
        <form name='confirma_tp_local' id='confirma_tp_local' action='fechamento.php?tploc=<?php echo $id_tp_local; ?>' method='POST'>
            
            <div id="loading" style="width:100%; height:96%; background-color:#000000; position:absolute; z-index:1; opacity:0.8;color:#FFFFFF; font-size:14px;" align="center">
            ...
            <div id="tpLocal" class="window" style="width:300px;height:200px;background-color:#eeeeee;margin-left:35%; margin-top:10%;" align="center">
            
                <div style="width:113%;height:45px;background-color:#f7b64b;margin-left:-20px; margin-top:-20px;color:#FFFFFF;font-family:Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size:18px;font-weight:bolder;" align="center">
                    <br />
                    Elegir el tipo de Local: 
                </div>
                ...
                <table align="center">
                    <tr align="center">
                        <td colspan="2">
                            <br />
                            <b style="color:#333333;">Tipo de Local:</b>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">&nbsp;
                    ...
                    <tr align="center">
                        <td colspan="2">
                            <select id="tpLocal" name="tpLocal">
                            
                                <?php
                                
                                    // buscar tipode de local.
                                    $sql_tipo_locais = "
                                        SELECT
                                            *
                                        FROM
                                    ...
                                    $query_tipo_locais = @mysql_query($sql_tipo_locais);									
    
                                    while ($res_tp_local=@mysql_fetch_assoc($query_tipo_locais))
                                    {
                                        echo "<option value='".$res_tp_local['id_tp_local']."'>".$res_tp_local['tp_local']."</option>";
                                    }								
                    ...
                    <tr align="center">
                        <td colspan="2">
                            ...
                            <br />
                            <br />
                            <input type="submit" id="btn_tp_local" name="btn_tp_local" value="Confirmar >>" style="width:200px;;background-color:#999999;color:#FFFFFF;font-size:14px;font-family:Verdana, Arial, Helvetica, sans-serif;"  />
                            </b>
                        </td>                    
                    </tr>                           
                </table>            
