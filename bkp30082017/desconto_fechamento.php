...
if (@mysql_num_rows( $query_a ) == 0) {
	...
	$acoes = array('Descontos'/*,'Diferenca'*/);
}
...
foreach ($acoes as $chave=>$valor) {
	...
	switch ($valor) {
		...
		case "Descontos":
				...
				$conteudo.="\n\t\t\t\t<input label='param_desc' type='hidden' name='param_desc' size='30' id='param_desc' value='".base64_encode('desconto_fech')."' />";
				...
				/*
				//bOleta ou fatura
				$conteudo.="\n\t\t\t\t<input type='checkbox' name='fatura' id='fatura' value='1' checked='checked' />
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function() {
			...
			$("#arq_desconto").click(function(event){
				...
				var id_leitura = <?= $dados_ult_leit['id_fechamento'] ?>;
				...
				$.post('add_registro.php', {param:vl_par,mot:id_desc,leit:id_leitura,valor:vl_desc,descri:descricao,fatu:tip_doc,numdoc:num_doc},function(json){
					...
					{
						$("#lc_nome").val("");
						if(confirm("deseja cadastrar desconto para esse fechamento?"))
						{
							alert(json);
							location="desconto_fechamento.php?id_ope=<?= $id_assoc ?>";
						}
						else
						{
							alert(json);
							location="pago_fechamento.php?id_ope=1";
						}
					}
				});
			});
			...
			$("#arq_diferenca").click(function(event){
				...
				$.post('add_registro.php', {param:vl_par,leit:id_leitura,valor:vl_dif,descri:descricao,maq_id:id_maq},function(json){
					...
					if(json=='Registro incluido com sucesso')
					{
						...
						{
							alert(json);
							location="desconto_fechamento.php?id_loc=<?= $dados_ult_leit['id_fechamento'] ?>";
						}
						else
						{
							alert(json);
