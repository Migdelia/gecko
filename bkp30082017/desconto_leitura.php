...
if (@mysql_num_rows( $query_a ) == 0) {
	...
	$acoes = array('Despesas','Diferenca');
}
...
foreach ($acoes as $chave=>$valor) {
	...
	switch ($valor) {
		...
		case "Despesas":
				...
				$conteudo.="\n\t\t\t\t<form name='create_desconto_fechamento' id='create_desconto_fechamento' action='add_registro.php' method='POST' target='_blank' enctype='multipart/form-data' onsubmit='return false;'>";
				...
				//bOleta ou fatura
				$conteudo.="\n\t\t\t\t<input type='checkbox' name='boleta' id='boleta' value='1' checked='checked' />
				<span style='color:#1D5987;font-family:Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size:12px;font-weight:bolder;' id='fatura'> Boleta &nbsp;</span>";
				
				//bOleta ou fatura
				$conteudo.="\n\t\t\t\t<input type='checkbox' name='fatura' id='fatura' value='1' />
				<span style='color:#1D5987;font-family:Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size:12px;font-weight:bolder;' id='fatura'> FATURA &nbsp;</span>";
				
				//bOleta ou fatura
				$conteudo.="\n\t\t\t\t<input type='checkbox' name='vale_por' id='vale_por' value='1' />
				<span style='color:#1D5987;font-family:Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size:12px;font-weight:bolder;' id='fatura'> Vale Por &nbsp;</span><br/><br />";											
				*/

				$conteudo.="\n\t\t\t\t   
				<label>
				<input type='radio' name='radio' id='fatura' value='fatura' checked='checked' />
				Fatura</label>
				<label>
				<input type='radio' name='radio' id='boleta' value='boleta' />
				Boleta</label>
				<label>
				...
				$conteudo.="\n\t\t\t\t<div class='lbl' id='lbl_desc'>Num Doc:</div>";
				$conteudo.="\n\t\t\t\t<input label='Numero Doc' type='text' name='num_doc' size='10' id='num_doc' title='Informe o numero do documento' value='' onfocus=\"$('#info_num_doc').css({'visibility':'visible'});\" onblur=\"$('#info_num_doc').css({'visibility':'hidden'});\"><br/>";
				
				//Informações do Nível
				$conteudo.="\n\t\t\t\t<div class='lbl' id='lbl_desc'>Valor desconto:</div>";
				$conteudo.="\n\t\t\t\t<input label='desconto' type='text' name='desconto' size='10' id='desconto' title='Informe o valor do desconto' value='' onfocus=\"$('#info_desc_fechamento').css({'visibility':'visible'});\" onblur=\"$('#info_desc_fechamento').css({'visibility':'hidden'});\"><br/>";
		...
		case "Diferenca":
				$conteudo.="\n\t\t\t<div id='tab".$chave."' class='tab_content'>";
				$conteudo.="\n\t\t\t\t<form name='create_diferenca_leitura' id='create_diferenca_leitura' action='add_registro.php' method='POST' target='_blank' enctype='multipart/form-data' onsubmit='return false;'>";
				$conteudo.="\n\t\t\t\t<input label='param_dif' type='hidden' name='param_dif' size='30' id='param_dif' value='".base64_encode('diferenca')."' />";
				$conteudo.="\n\t\t\t\t<span style='color:#414A4F;font-family:Verdana, Arial, Helvetica, sans-serif; font-size:18px;font-weight:bolder; text-transform:uppercase;'>";
				$conteudo.="\n\t\t\t\t\tDiferenca Leitura:";
				$conteudo.="\n\t\t\t\t</span>";
				$conteudo.="\n\t\t\t\t<hr style='background-color:#C2006E;border:none;height:3px;margin-bottom:4px;' />";
				...
				//Montando as Combos de maquinas.
				$mot = '';
				while ( $dados_maq=@mysql_fetch_assoc($query_maq) ) {
					$mot.= "\n\t\t\t\t\t<option value='".$dados_maq['id_maquina']."'>".$dados_maq['numero']."</option>";
				}
				$conteudo.="\n\t\t\t\t<div class='lbl' id='lbl_maq_desc'>Maquina:</div>";
				$conteudo.="\n\t\t\t\t<select name='maq_desc' id='maq_desc'  style='width:72px;' onfocus=\"$('#info_maq_desc').css({'visibility':'visible'});\" onblur=\"$('#info_maq_desc').css({'visibility':'hidden'});\">";
				$conteudo.=$mot;
				$conteudo.="\n\t\t\t\t</select>";
				$conteudo.="\n\t\t\t\t<label class='txt_info' id='info_maq_desc'>Maquina da diferenca.</label><br>";
				
				//Informações do Nível
				$conteudo.="\n\t\t\t\t<div class='lbl' id='lbl_desc'>Valor diferenca:</div>";
				$conteudo.="\n\t\t\t\t<input label='diferenca' type='text' name='diferenca' size='10' id='diferenca' title='Informe o valor do desconto' value='' onfocus=\"$('#info_dif_fechamento').css({'visibility':'visible'});\" onblur=\"$('#info_dif_fechamento').css({'visibility':'hidden'});\"><br/>";

				//Informações do Nível
				$conteudo.="\n\t\t\t\t<div class='lbl' id='lbl_desc_desconto'>Descricao:</div>";
				$conteudo.="\n\t\t\t\t<input label='descricao' type='text' name='descricao_dif' size='40' id='descricao_dif' title='Informe a descricao da diferenca' value='' onfocus=\"$('#info_desc_desconto').css({'visibility':'visible'});\" onblur=\"$('#info_desc_desconto').css({'visibility':'hidden'});\"><br/>";
				$conteudo.="\n\t\t\t\t<label class='txt_info' id='info_desc_desconto'>Descricao (livre) da diferenca.</label><br>";				
				$conteudo.="\n\t\t\t\t<button id='arq_diferenca' type='button' class='bt-enviar' style='margin-left:220px;'>Cadastrar</button>";				
				$conteudo.="\n\t\t\t\t</form>";				
				$conteudo.="\n\t\t\t\t<br clear='both' />";
			$conteudo.="\n\t\t\t</div>";
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function() {
			...
			$("#arq_desconto").click(function(event){
				...
				var descricao =$("#descricao").attr("value");
				var num_doc =$("#num_doc").attr("value");
				var fatura =$("#fatura").attr("checked");
				var boleta =$("#boleta").attr("checked");
				var vale =$("#vale").attr("checked");
				
				//verifica tipo de doc
				if(fatura==true)
				{
					tip_doc = "fatura";
				}
				...
				$.post('add_registro.php', {param:vl_par,mot:id_desc,leit:id_leitura,valor:vl_desc,descri:descricao,fatu:tip_doc,numdoc:num_doc},function(json){
			...
			//Desconto
			$("#arq_diferenca").click(function(event){
				event.preventDefault();
				var vl_par =$("#param_dif").attr("value");
				var id_leitura = <?= $dados_ult_leit['id_leitura'] ?>;
				var vl_dif =$("#diferenca").attr("value");
				var descricao =$("#descricao_dif").attr("value");
				var id_maq =$("#maq_desc").attr("value");

				$('<div id="agd_proc_regiao" style="text-decoration:blink;text-align:center; width:520px;">... Processando ...<br /><img src="img/aguarde.gif" align="aguarde" alt="aguarde"></div>').appendTo('#create_nivel');
				$("#arq_local").attr('disabled','disabled');
				$.post('add_registro.php', {param:vl_par,leit:id_leitura,valor:vl_dif,descri:descricao,maq_id:id_maq},function(json){
					$("#arq_desconto").attr('disabled','');
					$("#agd_proc_reg").remove();
					if(json=='Registro incluido com sucesso')
					{
