		...
		if($_SESSION['usr_nivel'] == 11)
		{
			...
			$sql_maq = "
				...
				AND
					...
					or
					leitura_por_maquina.id_local = 101)					
...
<th style='color:#E17009;'>Total</th>
		...
		while($result_maq=@mysql_fetch_assoc($query_maq)) {
			...
			*/
...

			{
				...
				$guias_cont.="\n\t\t\t\t<div id='resdif_".$result_dif['id_desconto']."' width='100%' align='center' style='display:none;'>";				
	...
	$guias_cont.="\n\t\t\t</table><br clear='both' />&nbsp;\n";
		...
		$guias_cont.="\n\t\t\t\t\t\t<tr bgcolor='#F5F8F9' height='21px;' style='font-weight:bolder' align='center'><th style='color:#E17009;' colspan='2'>Totalizadores<br />Lan&ccedil;amentos: </th>";
			...
			if($result_num_leit['id_local'] == 49 or $result_num_leit['id_local'] == 55 or $result_num_leit['id_local'] == 101)

			{

				$comissao = ($tot_sub_tot * 6) / 100;
			...
			$query_diferenca=@mysql_query($sql_diferenca);

			$result_diferenca=@mysql_fetch_assoc($query_diferenca);			


			//echo $result_desconto['desconto'];
			...
			$diferenca = $result_diferenca['desconto'];
			...
			$guias_cont.="\n\t\t\t\t\t\t\t&nbsp; <input id='deve' value='".number_format($diferenca,0,"",".")."' style='font-family:Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size:10px;font-weight:bolder;text-align:center;background-color:transparent; border:none;' readonly='readonly'   > ";//.number_format($vtot,'2',',','.');
			...
			//COMENTADO PARA LOCAL PROPRIO SEM SOCIO

			//$tot_cala =  ((($tot_tot_fat - $comissao - $rendicao) * 50) /100) + $tot_maq_cala;

			$tot_cala =  $tot_tot_fat - $comissao - $rendicao;
...
<head>
	...
	{
		var idDif = obj.id.split("_");
		$(document).ready(function() {	
			$('#resdif_'+idDif[1]).slideToggle('slow');
		});				
	}
