...
if( ($_SESSION['usuario']==$existe['usuario'])&&(md5($_SESSION['passw'])==md5($existe['senha']))&&($_SESSION['usuario']!='')&&($_SESSION['passw']!='') ){
	...
	if(@mysql_num_rows( $query_pg ) >= 1) {
		...
		if ( (@mysql_num_rows($query_a)== 0)||(strtoupper($result_pg['excluido'])=='S') ) {
			header('Location:sair_qr.php');
		}
	}
}else{
	session_destroy();
	$jsc ="<script language='javascript' type='text/javascript'>";
	$jsc.="\n\twindow.location.href='sair_qr.php?s=" .$senhaQR. "&v=". $codVer ."';";
	$jsc.="\n</script>";
	$des ="<a href='index_qr.php' title='Sair do sistema' style='text-decoration:none'><strong><img src='img/alerta.png' title='Sair do sistema' align='absmiddle' border=none alt='Sair do sistema' /> Sair do Sistema</strong></a>";
	exit($jsc);
}
?>
